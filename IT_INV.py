...
it_inv = it_inv_raw[(it_inv_raw["Status (Phy.)"] == "Yes") & (it_inv_raw["Status (Fixed Asset)"] != "No")].copy()
...
fin_inv = fin_inv_raw[["Business Unit AM", "Asset ID", "Asset Description", "Custodian", "Department"]].copy()
fin_inv["Asset ID"] = fin_inv.loc[:, "Asset ID"].str[-8:]
fin_inv["AssetCode"] = fin_inv["Business Unit AM"].astype(str) + fin_inv.loc[:, "Asset ID"].astype(str)
